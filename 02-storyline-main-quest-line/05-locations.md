# Story / Quest Locations

## Main Story / Quest Locations

### Pavlovo Military Base
- Outbreak origin.
- Classified biological experiment and containment breach.

### Balota Air Base Bunker
- Recovery operation endpoint.
- Biological material and advanced weapons-development equipment secured here.

### Rify Bunker / Port / Shipwreck
- NMR Base of Operations in Chernarus.
- Bunker = command / secure operations.
- Shipwreck contaminated zone = active research environment.
- Dedicated port = secure maritime logistics.

### Kamensk Military Base
- Confirmed Season 2 Story / Quest Line location.
- Exact story role and progression placement will be defined later.

### East Coast Bunker
- Confirmed Season 2 Story / Quest Line location.
- Exact story role and progression placement will be defined later.

### Tisy Military Base — Nuclear Launch Site Bunker
- High-tier / late-game quest location.
- Exact story role still pending.

## Military Areas Intentionally Outside Main Questline

These areas remain available for general events and player loot runs, but should not be mandatory Season 2 main-quest locations:

- Zelenogorsk Military Base
- VMC / Veresnik Military Base
- NWAF / Vybor Air Base

## Open Location Topics — PENDING TEAM DISCUSSION

- Additional custom locations may be created for Season 2 where the story or quest progression requires new environments.
- Existing custom locations may be revised, expanded or repurposed to better support the Season 2 storyline and quest flow.
- The final number, purpose and placement of any new or revised locations are not yet fixed and will be decided together with the team as the Story / Quest Line develops.
